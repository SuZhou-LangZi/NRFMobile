...
angular.module('com.amarsoft.mobile.controllers.RFexamine', ['ngSanitize'])
	...
	function ($scope, $state, $timeout, $ionicViewSwitcher, $rootScope, $stateParams, $http, $ionicPopup, $ionicLoading, basePage, $ionicScrollDelegate, $ionicModal) {
		...
		$scope.$on('To-refreshOpinion', function (e, data) {
			...
			// 执行提交操作
			if (data && data.ButFlag == 'DoSubmit') {
				console.log(data.ButFlag)
				$scope.DoSubmitX();
			}
			if(data && data.ButFlag == 'BatDoSubmit'){
				$scope.DoSubmitX();
			}
		...
		$scope.SaveOpinion = function () {
			$scope.$broadcast("To-saveOpinion", { ButtonFlag: 'SaveOpinion' });
		}
		$scope.saveOpinionList = function () {
			$scope.$broadcast("To-saveOpinionList", { ButtonFlag: 'saveOpinionList' });
		}
		...
		$scope.DoSubmit = function () {
			// if($scope.FlowNo == "BatCreditApproveFlow"){
			// 	$scope.$broadcast("To-saveOpinionList", { ButtonFlag: 'BatDoSubmit' });
			// } else{
			// 	$scope.$broadcast("To-saveOpinion", { ButtonFlag: 'DoSubmit' });
			// }
			$scope.DoSubmitX();
		}

		//提交下一阶段
		$scope.DoSubmitX = function () {
				...
				function (data, status) {
					...
					//批量BatCreditApproveFlow特殊流程跳转
						...
						if ("MEETING" == Params["attribute10"]) {
							...
							if (AmApp.config.sysSelType == '020') {
								// 金融板块
								$ionicModal.fromTemplateUrl(
									'templates/RFexamine/FMDoSubmit.html', {
										scope: $scope,
										backdropClickToClose: false
									}).then(function (modal) {
										$scope.modal = modal;
	...
	function ($scope, $state, $stateParams, $http, $ionicPopup, $ionicLoading, basePage) {
		$scope.$on('To-BasicTempletController', function (e, data) {
			console.log("通用模板加载");
			$scope.FlagType = 'Common';
			$scope.Param = data.Param; //参数
			//console.log($scope.Param);
			$scope.TempletType = data.TabMothod.split("_")[1];
			//console.log($scope.TempletType);
			if($scope.TempletType == 'IRSApplyInfo') {
				// 判断是哪个申请类型
				$scope.FlagType = 'IRSApplyInfo';
				$scope.items = {};
				$scope.ObjectNo = angular.fromJson($scope.Param);
				//console.log($scope.ObjectNo.attribute6);
				basePage.init($scope, loadCreditData);
			} else {
				$scope.FlagType = 'Common';
				$scope.items = [];
				basePage.init($scope, loadData);
		...
		//评级申请信息数据加载
		var loadCreditData = function ($ionicLoading) {
			runServiceWithSession(
					$http,
					$ionicLoading,
					$ionicPopup,
					...
					"RateApply", {		
						UserId: AmApp.userID,
						ObjectNo:$scope.ObjectNo.attribute6,
						ObjectType:$scope.ObjectNo.attribute7,
					},
					function (data, status) {		
						appIonicLoading.hide();
						console.log(data);
						$scope.items = angular.fromJson(data["array"][0].ObjectAll);												
					}
	...
	function ($scope, $rootScope, $state, $stateParams, $timeout, $http, $ionicPopup, $ionicLoading, basePage, $compile) {
			...
			runServiceWithSession(
				...
				function (data, status) {
					...
					if ($scope.BA.IRSEXPIRYDATE && $scope.TempletNo.indexOf("ApplyOpinionInfoEnt") < 0
							&& $scope.TempletNo.indexOf("ApplyOpinionInfoIndEx") < 0) {
					...
					if ($scope.TempletNo == 'ApplyOpinionInfo') {
		...
		//保存意见
		var SaveOpinion = function (eventData) {
			console.log($scope.RequiredArray);
			//检查详情必输项校验
			$scope.nullFalg = 0;
			...
			for (var i in $scope.BA) {
				...
				if ($scope.BA[i] && !((typeof $scope.BA[i]) == "number")) { //类型是number不进行特殊字符校验
					...
					if (result) {
						...
						$timeout(function () {
							$scope.$emit("To-refreshOpinion", { ButFlag: eventData.ButtonFlag }); //刷新意见信息
						}, 1500);
					}
				}
			}
	...
	.controller(
	'BasicHtmlController',
	function ($scope, $state, $stateParams, $http, $ionicPopup, $ionicLoading, basePage, $compile, $cordovaInAppBrowser, $sce) {
		$scope.htmlReportFlag = 'Noraml';
			...
			runServiceWithSession(
				...
				function (data, status) {
					...
					if ("NOTEXIST" == data["array"][0].BA) {
						$scope.noData = true;
					} else if ("URLACTION" == data["array"][0].BA) {
						$scope.noData = false;
						var url = data["array"][0].html;
						// App单点跳转
						var options = {
							location: 'no',
							clearcache: 'yes',
					...
					}  else if ("ENTEXIST" == data["array"][0].BA) {
						$scope.htmlReportFlag = 'Special';
						// 集团申请信息报告
						$scope.noData = false;
						var htmlJson = angular.fromJson(data["array"][0].html);
						$scope.reportInfo = htmlJson;
						// console.log(htmlJson.extobj[0]);
						// console.log(htmlJson);

					} else if ("FAILED" == data["array"][0].BA) {
						$scope.noData = true;
						appIonicLoading.show({
							template: "尽职调查报告还未生成，请先生成尽职调查报告再查看！",
							duration: 1500
						});
	...
	function ($scope, $state, $stateParams, $http, $ionicPopup, $ionicLoading, basePage) {
		...
		$scope.showActionUser = function (Action) {
			$scope.ActionUser = [];
			document.getElementById(Action.id).checked=true;
			...
			runServiceWithSession(
				...
				function (data, status) {
					...
					$scope.ActionSetList = nextPhaseInfo.ActionSetList;
					$scope.ActionUsers = nextPhaseInfo.PhaseAction;

					$scope.ActionListFlag = false;
					console.log($scope.ActionSetList);
					if ($scope.ActionSetList) {
						$scope.ActionListFlag = true;
						$scope.ActionSet = $scope.ActionSetList[0].ActionSet;
						$scope.ActionSet0 = $scope.ActionSetList[1].ActionSet;
					}
					console.log($scope.ActionUsers);

					$scope.ActionList0 = [];
					$scope.ActionList1 = [];
					for(var k=0; k<$scope.ActionUsers.length; k++) {
						if ($scope.ActionUsers[k]['UserNo'] != undefined) {
							if ($scope.ActionUsers[k].UserNo == 1) {
								$scope.ActionList1.push($scope.ActionUsers[k]);
							} else if ($scope.ActionUsers[k].UserNo == 0){
								$scope.ActionList0.push($scope.ActionUsers[k]);
							}
						}
					}
                    ...
                    // 
					if($scope.ActionUsers!=undefined&&$scope.ActionUsers!=undefined!=null&&$scope.ActionUsers!=""){
						for(var i=0;i<$scope.ActionUsers.length;i++){
							$scope.ActionUsers[i].id = "a"+i;
						}
					}
		...
		$scope.ActionUserChoose = function (User) {
			...
			if(document.getElementById(User.UserID)) {
				...
				if ($scope.ActionUser == undefined || $scope.ActionUser.length == 0) {
					$scope.ActionUser = [];
					$scope.ActionUser.push(User);
				} else {
					// 增加判断，如果已经存在同一阶段的用户则取最新的数据
					for (var j=0; j< $scope.ActionUser.length; j++) {
						if ($scope.ActionUser[j].UserStr.split('|')[0] == User.UserStr.split('|')[0] ) {
							$scope.ActionUser.splice(j, 1);
							$scope.ActionUser.push(User);
						} else {
							$scope.ActionUser.push(User);
						}
		...
		$scope.submit = function () { //必输校验、校验意见签署、校验是否已提交、不能提交给自己等都放在后端校验
			...
			if($scope.multiSelect) {
				var val0 = $("input:radio[name='nextActionOperatorItem0']:checked").val();
				var val1 = $("input:radio[name='nextActionOperatorItem1']:checked").val();
	...
	function ($scope, $state, $ionicModal, $rootScope, $stateParams, $http, $ionicPopup, $ionicLoading, basePage, $ionicScrollDelegate) {
		...
		var loadData = function ($ionicLoading) {
			runServiceWithSession(
				...
				function (data, status) {
					...
					for (var i = 0; i < data["array"].length; i++) {
						data["array"][i].ShowFlag = true;
						if (data["array"][i].PhaseOpinion1 == '同意') {
							data["array"][i].PhaseOpinion1 = '1';
						} else if (data["array"][i].PhaseOpinion1 == '否决') {
							data["array"][i].PhaseOpinion1 = '2';
						} else if (data["array"][i].PhaseOpinion1 == '退回补充资料') {
							data["array"][i].PhaseOpinion1 = '3';
						}
						$scope.items.push(data["array"][i]);
					}
					$scope.loadingMore = false;
		...
		//保存
		var saveOpinionList = function (eventData) {
